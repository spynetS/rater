...
def add_friend(request,account_id):
    ...
    account.friends.add(get_object_or_404(Account,pk=account_id))
    account.save()
    response = HttpResponse()
    response.headers['addedfriend'] = True
    return response

@login_required
def remove_friend(request,account_id):
    account:Account = get_object_or_404(Account,user=request.user)
    account.friends.remove(get_object_or_404(Account,pk=account_id))
    account.save()

    response = HttpResponse()
    response.headers['removedfriend'] = True
    return response
